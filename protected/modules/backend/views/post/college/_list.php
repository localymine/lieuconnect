<?php foreach ($posts as $row): ?>
    ...
    <li class="" data-postid="<?php echo $row->id ?>">
        ...
        <div class="hold-img-news">
            <img class="img-news" align="left" width="126" style="padding:5px" src="<?php echo Yii::app()->baseUrl ?>/images/college/<?php echo $row->image ?>" />
        </div>
        <div><?php echo (strlen($row->post_title) < 32) ? $row->post_title : mb_substr($row->post_title, 1, 29) . '...' ?><br/><?php echo $row->award ?></div>
        <a href="<?php echo Yii::app()->createUrl('backend/post/updateCollege/' . $row->id) ?>" class="ui-icon-edit pull-left pl10" title="Edit"><i class="fa fa-pencil"></i></a>
