...
<div class="content-header">
    <div class="header-section">
        <h1>
            <i class="gi gi-brush"></i>All Internship<br />
            <?php
    ...
    <div class="col-md-12">
        <div class="block">
            <div class="block-title">
                <h2>List Internship</h2><div class="block-options pull-right"><i class="fa fa-star" style="color:#1BBAE1"></i>Show   <i class="fa fa-star" style="color:#ccc"></i>Hidden</div>
...
//                                                    $provided_by = 'provided_by' . $suffix;
                                                    ...
                                                    ?>
                                                    <img align="left" width="126" style="padding:5px" src="<?php echo Yii::app()->baseUrl ?>/images/internship/<?php echo $row->{$image} ?>" />
                                                    <div><?php echo $row->{$post_title} ?></div>
                                                    <!--<div>Provided By <?php // echo $row->{$provided_by} ?></div>-->
                                                    <!--<div>Award: <?php // echo $row->award ?></div>-->
                                                    <div>Internship Deadline: <?php echo $row->deadline ?></div>
    ...
    <?php endforeach; ?>
                                ...
                                <td>
                                    <div class="block-options">
                                        ...
                                        <a title="" data-toggle="tooltip" class="btn btn-alt btn-sm btn-default btn-option" href="<?php echo Yii::app()->createUrl('backend/post/updateInternship', array('id' => $row->id)) ?>" data-original-title="Edit"><i class="fa fa-pencil"></i></a>
...
<script>
    $(function() {
        ...
        $('a.status').click(function(e) {
            ...
            var request = $.ajax({
                url: '<?php echo Yii::app()->createUrl('backend/post/statusInternship') ?>',
                type: 'POST',
                data: {id: postid},
                dataType: 'json',
                beforeSend: function() {
        ...
        $("a.delete").click(function(e) {
            ...
            if (confirm('Are you sure you want to delete this?\nCan not recover after delete.')) {
                var postid = $(this).data('postid');
                var request = $.ajax({
                    url: '<?php echo Yii::app()->createUrl('backend/post/deleteInternship') ?>',
                    type: 'POST',
                    data: {id: postid},
                    dataType: 'json',
                    beforeSend: function() {
