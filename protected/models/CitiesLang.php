...
class CitiesLang extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('cityID, lang_id', 'required'),
			array('cityID', 'numerical', 'integerOnly'=>true),
			array('lang_id', 'length', 'max'=>6),
			array('l_cityName', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('l_id, cityID, lang_id, l_cityName', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('l_cityName',$this->l_cityName,true);
